...
import { Play, FileEdit, Plus, Layers, X, Github } from 'lucide-react';
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8">
                    <div className="w-full max-w-6xl mx-auto space-y-6 md:space-y-8 pb-12">
                        ...
                        <AnimatePresence>
                            {!checklistDismissed && (
                                <motion.div initial={{ opacity: 0, height: 0, scale: 0.95 }} animate={{ opacity: 1, height: 'auto', scale: 1 }} exit={{ opacity: 0, height: 0, scale: 0.95 }} className="w-full">
                                    <div className="bg-[#111] border border-[#222] p-4 md:p-6 relative rounded-xl">
                                        <button onClick={handleDismissChecklist} className="absolute top-3 right-3 text-[#64748B] hover:text-[#F1F5F9] transition-colors">
                                            <X className="w-4 h-4" />
                                        </button>
                                        <h3 className="font-mono text-xs md:text-sm text-[#6EE7B7] mb-4 md:mb-6">getting_started</h3>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                                            {checklistItems.map((item) => (
                                                <div key={item.id} onClick={() => handleChecklistClick(item)} className={`flex items-center gap-2 md:gap-3 font-mono text-xs md:text-sm transition-colors ${item.locked ? 'opacity-50 cursor-not-allowed text-[#64748B]' : item.done ? 'text-[#6EE7B7] cursor-default' : 'text-[#64748B] hover:text-[#F1F5F9] cursor-pointer'}`}>
                                                    {item.done ? <span className="text-[#6EE7B7]">✓</span> : <span className="text-[#64748B]">○</span>}
                        ...
                        <motion.div variants={containerVariants} initial="hidden" animate="show" className="pt-2 md:pt-4">
                            ...
                            <div className="bg-[#111] border border-[#222] rounded-xl p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-6">
                                <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        {selectedRepo ? (
                                            <>
                                                <h3 className="text-sm md:text-base font-mono font-semibold text-[#F1F5F9] mb-0.5 md:mb-1 truncate">{selectedRepo.full_name}</h3>
                                                <span className="flex items-center gap-2 text-[10px] md:text-xs font-mono text-[#6EE7B7]">
                                                    <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-xl bg-[#6EE7B7] animate-pulse"></span> connected
                                                </span>
                                            </>
                                        ) : (
                                            <>
                                                <h3 className="text-sm md:text-base font-mono font-semibold text-[#F1F5F9] mb-0.5 md:mb-1">No repository connected</h3>
                                                <span className="text-[10px] md:text-xs font-mono text-[#64748B] line-clamp-1">
                                ...
                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 md:gap-3 w-full md:w-auto relative">
                                    {selectedRepo ? (
                                        <>
                                            <Button variant="ghost" onClick={() => setShowRepoSelector(!showRepoSelector)} className="font-mono text-xs md:text-sm border border-[#222] text-[#F1F5F9] rounded-xl w-full sm:w-auto justify-center">
                                                Change Repo
                                            </Button>
                                            <Button variant="primary" className="gap-2 bg-[#6EE7B7] text-[#080808] hover:bg-[#34D399] border-none font-bold rounded-xl text-xs md:text-sm w-full sm:w-auto justify-center" onClick={() => navigate(`/workflows/new?repo=${selectedRepo.full_name}`)}>
                                                Create Workflow →
                                            </Button>
                                        </>
                                    ) : (
                                        <Button variant="primary" className="gap-2 bg-[#6EE7B7] text-[#080808] hover:bg-[#34D399] border-none font-bold rounded-xl w-full sm:w-auto justify-center text-xs md:text-sm" onClick={() => setShowRepoSelector(!showRepoSelector)}>
                                            Select Repository →
                                        </Button>
                                    )}
                                    ...
                                    <AnimatePresence>
                                        {showRepoSelector && (
                                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 left-0 md:left-auto top-full mt-2 w-full md:w-64 bg-[#111] border border-[#222] shadow-xl z-50 p-3 flex flex-col gap-2 rounded-xl">
                                                {!isGithubConnected ? (
                                                    ...
                                                    <>
                                                        <div className="text-[10px] md:text-xs font-mono text-[#64748B] mb-1 px-1">Select repository</div>
                                                        <select className="w-full bg-[#0A0A0A] border border-[#222] rounded-xl text-xs font-mono text-[#F1F5F9] outline-none px-3 py-2 cursor-pointer focus:border-[#444] hover:border-[#333]" value={selectedRepo?.full_name || ''} onChange={handleRepoSelect}>
                                                    ...
                                                    <motion.tr key={workflow.id} variants={itemVariants} className="hover:bg-[#111] transition-colors group">
                                                        ...
                                                        <td className="pl-4 py-3 md:pl-6 md:py-4 whitespace-nowrap text-right">
                                                            <div className="flex justify-end gap-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                                                <Link to="/workflows/new" className="text-[#64748B] hover:text-[#6EE7B7] transition-colors p-1" title="Edit">
                                                                    <FileEdit className="w-3.5 h-3.5 md:w-4 md:h-4" />
                                                                </Link>
                                                                <button className="text-[#64748B] hover:text-[#6EE7B7] transition-colors p-1" title="Run">
